...
add_action('plugins_loaded', function ()
{
  ...
  class WC_Integration_Valutec extends WC_Integration
  {
    public function __construct()
    {
      ...
      $this->method_description = '<p>Integrates WooCommerce payments with ' . 'Valutec\'s tender API. Just paste the API keys that Valutec gives ' . 'you into the forms below. Customers can enter gift cards in the ' . 'cart screen and the total will be adjusted and the card charged at ' . 'checkout. Notes are added to your order in the backend so that you ' . 'have the transaction IDs.</p>' . '' . '<p>Cache type: <code>' . WC_Valutec_Cache::$type . '</code></p>';
  ...
  add_action('woocommerce_init', function ($wc)
  {
    ...
    {
      if ($xml = $api_call('Transaction_Sale', $id, $code, array(
        'Amount' => $amount
      )))
      {
    ...
    {
      if ($xml = $api_call('Transaction_Void', $id, $code, array(
        'RequestAuthCode' => $auth_code
      )))
      {
        $balance = (float)$xml->Balance;
        if ($xml->Authorized == 'false')
...
<?php
    ...
    add_filter('woocommerce_get_order_item_totals', function ($rows, $order) use ($shorten)
    {
      ...
      return array_slice($rows, 0, count($rows) - 1, true) + $coupon_entries + array_slice($rows, count($rows) - 1, 1, true);
    }
    , 10, 2);
    ...
    add_action($cart_hook, function ()
    { // TODO what if plugin is deactivated during order
...
?><div class="gift-cards" id="gift-cards">
    ...
    add_action($checkout_hook, function ()
    { // TODO what if plugin is deactivated during order
      if (WC()->cart->needs_payment())
      {
        $button_classes = apply_filters('woocommerce_valutec_gift_cards_btnclass', ['apply-gift-card', 'button']);
